...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const STYLE = `
      ...
      [data-dsh-mobile-remote-settings] .wxb-remote-actions { display: flex; flex-wrap: wrap; align-items: center; gap: 8px; }
      [data-dsh-mobile-remote-settings] .wxb-device-list { display: flex; width: min(560px, 100%); flex-direction: column; gap: 8px; }
      [data-dsh-mobile-remote-settings] .wxb-device { display: flex; align-items: center; gap: 12px; width: 100%; box-sizing: border-box; padding: 10px 12px; border: 1px solid var(--dsw-alias-border-l2); border-radius: 8px; background: var(--dsw-alias-bg-module-platform); }
      [data-dsh-mobile-remote-settings] .wxb-device-copy { min-width: 0; flex: 1; }
      [data-dsh-mobile-remote-settings] .wxb-device-name { color: var(--dsw-alias-label-primary); font-size: 13px; line-height: 20px; overflow-wrap: anywhere; }
      [data-dsh-mobile-remote-settings] .wxb-device-time { color: var(--dsw-alias-label-tertiary); font-size: 12px; line-height: 18px; }
    ...
    function MobileRemoteSettings() {
      ...
      useEffect(() => {
        installStyles()
        let active = true
        const load = () => request('GET').then((data) => {
          if (active) {
            setSnapshot(data)
            setError(null)
          }
      ...
      const runAction = async (action, values = {}) => {
        setBusy(true)
        try {
          const data = await request('POST', { action, ...values })
          if (data.snapshot) setSnapshot(data.snapshot)
          setError(null)
        } catch (err) {
      ...
      const statusClass = running ? 'wxb-pill-online' : remote.phase === 'error' ? 'wxb-pill-error' : 'wxb-pill-waiting'
      const devices = Array.isArray(remote.devices) ? remote.devices : []
      return h('section', { 'data-dsh-mobile-remote-settings': '' },
        h('div', { className: 'wxb-header' },
          h('div', null,
            h('h2', { className: 'wxb-title' }, '移动端远程'),
            h('p', { className: 'wxb-subtitle' }, '通过安全配对访问 DSH，手机端使用迁移后的竖屏导航与响应式界面。')),
          h(Button, {
        ...
        h('div', { className: 'wxb-card' },
          ...
          h('div', { className: 'wxb-status', role: 'status' },
            h('span', { className: `wxb-pill ${statusClass}` }, statusLabel),
            h('span', { className: 'wxb-status-text' }, remote.detail || '移动端远程当前未启用。')),
          h('div', { className: 'wxb-remote-actions' },
            h(Button, {
              variant: running ? 'outline' : 'primary',
              size: 'sm',
              disabled: busy || (!running && !remote.canStart),
              onClick: () => { void runAction(running ? 'stop-mobile-remote' : 'start-mobile-remote') },
            }, busy ? '正在处理…' : running ? '停止移动端远程' : '启动移动端远程'))),
        h('div', { className: 'wxb-card' },
          h('h3', null, '扫描配对'),
          h('p', null, running ? '二维码五分钟内有效；扫码后设备会获得独立会话令牌。' : '启动移动端远程后，使用手机扫描此处二维码。'),
          remote.pairingQrImage
            ? h('img', { className: 'wxb-qr', src: remote.pairingQrImage, alt: '移动端远程配对二维码' })
            : h('div', { className: 'wxb-remote-qr', role: 'img', 'aria-label': '移动端配对二维码尚未生成' },
              running ? '正在生成配对二维码…' : '启动后生成配对二维码'),
          h('div', { className: 'wxb-address' }, lanUrl),
          h('div', { className: 'wxb-meta' }, `Reader：${readerUrl}`),
          running ? h(Button, {
            variant: 'outline', size: 'sm', disabled: busy,
            icon: h(IconRefreshOutline16, null),
            onClick: () => { void runAction('refresh-mobile-pairing') },
          }, '刷新配对二维码') : null,
          h('div', { className: 'wxb-device-list' },
            h('h3', null, `已配对设备（${devices.length}）`),
            devices.length === 0
              ? h('div', { className: 'wxb-empty' }, '尚无已配对设备。')
              : devices.map((device) => h('div', { className: 'wxb-device', key: device.id },
                h('div', { className: 'wxb-device-copy' },
                  h('div', { className: 'wxb-device-name' }, device.name || '移动设备'),
                  h('div', { className: 'wxb-device-time' }, `最近访问：${new Date(device.lastSeenAt || device.pairedAt).toLocaleString()}`)),
                h(Button, {
                  variant: 'outline', size: 'sm', disabled: busy,
                  icon: h(IconTrashOutline16, null),
                  onClick: () => { void runAction('revoke-mobile-device', { deviceId: device.id }) },
                }, '撤销')))),
