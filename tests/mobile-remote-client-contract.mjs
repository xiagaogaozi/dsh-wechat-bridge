...
assert.match(host, /MOBILE_REMOTE_DEVICES_FILE/, 'Paired-device token hashes must use a dedicated private plugin storage file.')
assert.match(host, /initialDevices:\s*initialMobileRemoteDevices/, 'The host must restore paired devices when the gateway is recreated.')
assert.match(host, /onDevicesChanged:\s*saveMobileRemoteDevices/, 'The host must persist gateway device mutations through the DSH file service.')
