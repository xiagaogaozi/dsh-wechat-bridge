...
assert.match(client, /http:\/\/本机局域网IP:3082/, 'The page must retain a readable fallback LAN address.')
assert.match(client, /Reader：\$\{readerUrl\}/, 'The page must display the Reader address below the pairing area.')

assert.match(host, /networkInterfaces \} from 'node:os'/, 'The host must derive the LAN address without shell commands.')
...
assert.match(host, /mobileRemoteDevices:\s*z\.array\(z\.object\(/, 'Paired-device summaries must be declared in the plugin settings schema.')
assert.match(host, /const loadMobileRemoteDevices = \(\) => Array\.isArray\(cfg\.mobileRemoteDevices\)/, 'The host must restore paired devices from its settings namespace when the gateway is recreated.')
assert.match(host, /settingsSvc\.update\('dsh-wechat-bridge', \{ mobileRemoteDevices: devices \}\)/, 'The host must persist gateway device mutations through DSH settings rather than the workspace-write file service.')
assert.doesNotMatch(host, /MOBILE_REMOTE_DEVICES_FILE|dsh-wechat-bridge-mobile-remote-devices\.json/, 'Paired-device persistence must not use a path the DSH file sandbox cannot write.')
assert.match(host, /onDevicesChanged:\s*saveMobileRemoteDevices/, 'The gateway must await the host persistence callback before issuing a pairing cookie.')
assert.match(host, /settingsSvc\.replace\('dsh-wechat-bridge', \{ mobileRemoteDevices: preservedMobileRemoteDevices \}\)/, 'Resetting bridge options must retain persisted paired-device summaries.')
...
assert.match(gateway, /existingDevice\s*=\s*requestDevice\(req\)/, 'A repeated scan in the same browser must reuse its existing device.')
assert.match(gateway, /await persistDevices\(\)/, 'Pairing and revocation must await durable device persistence.')
assert.doesNotMatch(client, /dsh-wechat-bridge\/mobile-ui|mobile-nav-toggle/, 'The bridge client must not embed the optional mobile UI.')
assert.doesNotMatch(client, /Mobile UI adapted from|dsh-web-mobile/, 'The generated bridge client must not carry the removed mobile UI.')
assert.doesNotMatch(client, /--aion-|--ds-ease/, 'The settings client must use DSH --dsw-* theme variables only.')
